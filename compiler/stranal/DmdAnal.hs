 ...
 * it makes the strictness signatures, and hence slows down
   fixpointing and
 * it is useless any ways: For lazy, used-many times fv's we will never get any
   better result than that, no matter how good the demand on the function at
   the call site is.
Therefore we exclude lazy multiple-used fv's from the environment in the
DmdType. But now the signature lies! (Missing variables are assumed to be
